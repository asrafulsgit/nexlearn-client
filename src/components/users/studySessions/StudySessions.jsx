...
const StudySessions = () => {
  ...
  const { data, isPending, isError, error, refetch } = useQuery({
    ...
    queryFn: () =>
      apiRequiest("get", `/sessions/user?page=${activePage}&limit=${limit}`),
    ...
    interval = setTimeout(async () => {
      try {
        const data = await apiRequiest(
          "get",
          `/sessions/user/search?name=${searchValue}`,
        );
      ...
      <section className="min-h-screen bg-gray-50 py-10">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Discover and join collaborative study sessions that match your
        ...
        {/* Search and Filters */}
        <div
          className="bg-white rounded-lg shadow p-6 mb-12 flex 
                    ...
                    <div className="flex flex-col justify-between flex-grow p-3">
                      <div>
                        <h3 className="text-xl font-semibold text-gray-800 mb-2">
                          {session?.title.length > 23
                            ? `${session?.title.slice(0, 23)}...`
                            : session?.title}
                        </h3>
                      ...
                      <div className="flex items-center justify-between mt-auto pt-4 border-t border-gray-200">
                        <span className="text-sm text-gray-500">
                          Starts: {dateFormat(session?.classStart)}
                        </span>
                        <Link
